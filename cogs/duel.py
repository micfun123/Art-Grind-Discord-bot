...
class pyduel(commands.Cog):
    ...
    @commands.slash_command(name="duel", description="Start a duel with another user. (duration_unit can be minutes or days)")
    async def duel(self, ctx, topic: str, duration: int, duration_unit: str):
        ...
        sleep_duration = duration * 60 if duration_unit.lower() == 'minutes' else duration * 60 * 24
        ...
        for i, submission in enumerate(submissions):
            ...
            await channelsend.send(f"{reaction_emojis[i]}: {submission['url']} (Made by: {submission['author'].mention})")
