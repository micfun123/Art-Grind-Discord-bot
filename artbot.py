...
def generate_i_made(url):
    ...
    img.paste(
        im2.rotate(30, resample=Image.BILINEAR, expand=1, fillcolor=(255, 255, 255, 0)),
        (1300, 450),
    )
    d = BytesIO()
    d.seek(0)
...
async def weekly_challenge():
    ...
    if today == 5:  # Monday == 7
        ...
        em = discord.Embed(
            title="Weekly Challenge", description=f"{message.content}", color=0x00FF00
        )
...
async def code(ctx):
    em = discord.Embed(
        title="You want my code",
        description="I dont give my code to any one you know",
        color=0x00FF00,
    )
    em.add_field(
        name="Link :", value="https://github.com/micfun123/Art-Grind-Discord-bot"
    )
    await ctx.respond(embed=em)


@client.slash_command(
    guild_ids=[856677753108693002],
    description="Suggest a theme for the weekly challenge",
)
async def suggest_theme(ctx, theme):
    em = discord.Embed(
        title="Suggestion",
        description=f"{theme}\n Suggested by: {ctx.author.name}",
        color=0x00FF00,
    )
...
async def duelidea(ctx):
    ...
    em = discord.Embed(
        title="Duel Idea",
        description="Here is a duel idea for you to try out!",
        color=0x00FF00,
    )
...
async def invite(ctx):
    await ctx.respond(
        f"{ctx.author.mention} here is your invite link: {await client.invite(ctx.guild)}"
    )
...
async def leaderboardscore(ctx):
    with open("score.json", "r") as f:
        ...
        em = discord.Embed(
            title="All Scores", description="Top 10 score board", color=0x00FF00
        )
...
async def StylePrompt_command(ctx):
    ...
    em = discord.Embed(
        title="Style Prompt. Have fun making", description=f"{myline}", color=0x20BEFF
    )
    await ctx.send(embed=em)
